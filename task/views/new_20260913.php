  ...
  <?php if($screenMode === '1') : ?>
    ...
    <form action="/tmp/task/create.php" method="post">
      ...
      <div>
        ...
        <div>
          <input type="radio" id="priority1" name="priority" value="低" <?php if($tasks['priority'] === '低') : ?>checked<?php endif; ?>>
          <label for="priority1">低</label>
        </div>

        <div>
          <input type="radio" id="priority2" name="priority" value="中" <?php if($tasks['priority'] === '中') : ?>checked<?php endif; ?>>
          <label for="priority2">中</label>
        </div>

        <div>
          <input type="radio" id="priority3" name="priority" value="高" <?php if($tasks['priority'] === '高') : ?>checked<?php endif; ?>>
      ...
      <div>
        ...
        <div>
          <input type="radio" id="status1" name="status" value="未着手" <?php if($tasks['status'] === '未着手') : ?>checked<?php endif; ?>>
          <label for="status1">未着手</label>
        </div>
        <div>
          <input type="radio" id="status2" name="status" value="進行中" <?php if($tasks['status'] === '進行中') : ?>checked<?php endif; ?>>
          <label for="status2">進行中</label>
        </div>
        <div>
          <input type="radio" id="status3" name="status" value="完了" <?php if($tasks['status'] === '完了') : ?>checked<?php endif; ?>>
    ...
    <a href="/tmp/task/index.php">一覧に戻る</a>
